    ...
    def compute_dv(self, node, max_depth_value):
        ...
        def level_contribution(self, node, max_depth_value):
            ...
            for k in range(len(level_counts)):
                nk = cumulative_count + level_counts[k]
                if k == 0:
                    ck = nk
                else:
                    ck = contributions[-1] + (nk - cumulative_count) * (max_depth_value - (k - 1)) / max_depth_value
...
if __name__ == "__main__":
    tree = LanguageFamilyTree("data/Afro-Asiatic.csv")
    tree.export_tree("data/Afro-Asiatic.json", format="json")